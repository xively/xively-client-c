...
typedef const xi_memory_limiter_entry_t *(
    xi_memory_limiter_entry_list_on_elem_t)(
    const xi_memory_limiter_entry_t *entry, const void *user_data);

static const xi_memory_limiter_entry_t *
xi_memory_limiter_entry_array_same_predicate(
    const xi_memory_limiter_entry_t *entry, const void *user_data) {
  return entry == user_data ? entry : NULL;
}

static xi_memory_limiter_entry_t *xi_memory_limiter_entry_list_visitor(
    xi_memory_limiter_entry_list_on_elem_t *predicate, const void *user_data) {
...
static xi_state_t xi_memory_limiter_will_allocation_fit(
    xi_memory_limiter_allocation_type_t memory_type, size_t size_to_alloc) {
...
size_t xi_memory_limiter_get_current_limit(
    xi_memory_limiter_allocation_type_t limit_type) {
  assert(limit_type <= XI_MEMORY_LIMITER_ALLOCATION_TYPE_COUNT);
...
size_t xi_memory_limiter_get_allocated_space() { return xi_memory_allocated; }

void *xi_memory_limiter_alloc(xi_memory_limiter_allocation_type_t limit_type,
                              size_t size_to_alloc, const char *file,
                              size_t line) {
  assert(limit_type < XI_MEMORY_LIMITER_ALLOCATION_TYPE_COUNT);
  assert(NULL != file);
  ...
  local_state =
      xi_memory_limiter_will_allocation_fit(limit_type, real_size_to_alloc);
...
void *xi_memory_limiter_realloc(xi_memory_limiter_allocation_type_t limit_type,
  ...
  const size_t real_size_to_alloc =
      size_to_alloc + sizeof(xi_memory_limiter_entry_t);
...
void *xi_memory_limiter_calloc_application(size_t num, size_t size_to_alloc,
                                           const char *file, size_t line) {
  return xi_memory_limiter_calloc(XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                  num, size_to_alloc, file, line);
}

void *xi_memory_limiter_realloc_application(void *ptr, size_t size_to_alloc,
                                            const char *file, size_t line) {
  return xi_memory_limiter_realloc(
      XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION, ptr, size_to_alloc, file,
      line);
}

void *xi_memory_limiter_alloc_system(size_t size_to_alloc, const char *file,
                                     size_t line) {
  return xi_memory_limiter_alloc(XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                 size_to_alloc, file, line);
}

void *xi_memory_limiter_calloc_system(size_t num, size_t size_to_alloc,
                                      const char *file, size_t line) {
  return xi_memory_limiter_calloc(XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM, num,
                                  size_to_alloc, file, line);
}

void *xi_memory_limiter_realloc_system(void *ptr, size_t size_to_alloc,
                                       const char *file, size_t line) {
  return xi_memory_limiter_realloc(XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                   ptr, size_to_alloc, file, line);
}

void *xi_memory_limiter_alloc_application_export(size_t size_to_alloc) {
  return xi_memory_limiter_alloc_application(size_to_alloc, "exported alloc",
                                             0);
}

void *xi_memory_limiter_calloc_application_export(size_t num,
                                                  size_t size_to_alloc) {
  return xi_memory_limiter_calloc_application(num, size_to_alloc,
                                              "exported calloc", 0);
}

void *xi_memory_limiter_realloc_application_export(void *ptr,
                                                   size_t size_to_alloc) {
  return xi_memory_limiter_realloc_application(ptr, size_to_alloc,
                                               "exported re-alloc", 0);
}
