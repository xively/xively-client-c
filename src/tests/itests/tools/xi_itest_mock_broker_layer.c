...
************************************************************************************/
xi_state_t xi_mock_broker_secondary_layer_push( void* context, void* data,
                                                xi_state_t in_out_state )
{
    ...
    if ( control == CONTROL_ERROR )
    {
        ...
        xi_evtd_execute_in(
            ...
            xi_make_handle( xi_itest_find_layer( xi_context, XI_LAYER_TYPE_MQTT_CODEC_SUT )->layer_funcs->pull,
                            &xi_itest_find_layer( xi_context, XI_LAYER_TYPE_MQTT_CODEC_SUT )->layer_connection,
                            0, in_out_state )
            , 1 );
    ...
    {
        // jump to libxively's codec layer pull function, mimicing incoming encoded message
        xi_evtd_execute_in(
            xi_globals.evtd_instance,
            xi_make_handle( xi_itest_find_layer( xi_context, XI_LAYER_TYPE_MQTT_CODEC_SUT )->layer_funcs->pull,
                            &xi_itest_find_layer( xi_context, XI_LAYER_TYPE_MQTT_CODEC_SUT )->layer_connection,
...
xi_state_t xi_mock_broker_secondary_layer_pull( void* context, void* data,
                                                xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_secondary_layer_close( void* context, void* data,
                                                 xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_secondary_layer_close_externally( void* context, void* data,
                                                            xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_secondary_layer_init( void* context, void* data,
                                                xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_secondary_layer_connect( void* context, void* data,
                                                   xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_layer_push( void* context, void* data,
                                      xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
    ...
    if (in_out_state == XI_STATE_OK)
    {
        ...
        xi_evtd_execute_in(
            xi_globals.evtd_instance,
            xi_make_handle( xi_itest_find_layer( xi_context_mockbroker, XI_LAYER_TYPE_MOCKBROKER_MQTT_CODEC )->layer_funcs->pull,
                            &xi_itest_find_layer( xi_context_mockbroker, XI_LAYER_TYPE_MOCKBROKER_MQTT_CODEC )->layer_connection,
                            data, XI_STATE_OK )
            , 1 );
...
xi_state_t xi_mock_broker_layer_pull( void* context, void* data,
                                      xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
    ...
    if ( in_out_state == XI_STATE_OK )
    {
        ...
        switch ( recvd_msg_type )
        {
            case XI_MQTT_TYPE_CONNECT:
            {
                ...
                XI_CHECK_STATE( in_out_state = fill_with_connack_data( msg_connack, XI_CONNACK_ACCEPTED ) );

                return XI_PROCESS_PUSH_ON_PREV_LAYER( context, msg_connack, in_out_state );
            }
            break;

            case XI_MQTT_TYPE_SUBSCRIBE:
            {
                const char* subsribe_topic_name = (const char*)recvd_msg->subscribe.topics->name->data_ptr;
                ...
                // note: fill subscribe can be used for fill subsck only because the memory map of the
                //       two structs are identical since unions are used
                XI_CHECK_STATE( in_out_state = fill_with_subscribe_data(
                    msg_suback,
                    "unused topic name", // topic name is not used in SUBACK, so this string is unrelevant
            ...
            {
                const char* publish_topic_name = (const char*)recvd_msg->publish.topic_name->data_ptr;
...
xi_state_t xi_mock_broker_layer_close( void* context, void* data,
                                          xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_layer_close_externally( void* context, void* data,
                                          xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_layer_init( void* context, void* data,
                                      xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
...
xi_state_t xi_mock_broker_layer_connect( void* context, void* data,
                                         xi_state_t in_out_state )
{
    XI_LAYER_FUNCTION_PRINT_FUNCTION_DIGEST();
