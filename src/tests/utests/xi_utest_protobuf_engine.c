...
ProtobufCAllocator xi_utest_protobuf_engine_allocator
    = {&xi_utest_protobuf_alloc, &xi_utest_protobuf_free, NULL};
...
XI_TT_TESTCASE(
    utest__xi_protobuf_engine__correct_data__serialise_credentials_deserialise,
    {
        ...
        rpc_header.checksum.len
            = sizeof( xi_utest_protobuf_engine_checksum_data );
        ...
        CTRPC* rpc_unpacked = ct__rpc__unpack(
            &xi_utest_protobuf_engine_allocator, bytes, packed_message );
        ...
        tt_want_str_op( rpc_message.header->msgid, ==,
                        rpc_unpacked->header->msgid );

        // check credentials
        tt_want_ptr_op( rpc_unpacked->sndcreds, !=, NULL );
        tt_want_str_op( rpc_message.sndcreds->cred, ==,
                        rpc_unpacked->sndcreds->cred );

        ct__rpc__free_unpacked( rpc_unpacked,
                                &xi_utest_protobuf_engine_allocator );

        tt_want_int_op( xi_is_whole_memory_deallocated(), >, 0 );
    } )

XI_TT_TESTCASE(
    utest__xi_protobuf_engine__correct_data__serialise_response_deserialise,
    {
        CTRPC rpc_message = CT__RPC__INIT;
        CTRPC__Header rpc_header = CT__RPC__HEADER__INIT;
        ...
        tt_want_int_op( rpc_message.response->result, ==,
                        rpc_unpacked->response->result );
