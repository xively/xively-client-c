...
xi_data_desc_t topic_name_desc
    = make_static_desc( topic_name, sizeof( topic_name ) - 1 );
xi_data_desc_t content_desc
    = make_static_desc( content, sizeof( content ) - 1 );

utest_mqtt_test_case_t array_of_test_case[]
    = {{// test case 0
        {.publish
         = {.common = {{.common_bits = {0, 0, 0, XI_MQTT_TYPE_PUBLISH}}, 0},
            &topic_name_desc,
            0,
            &content_desc}},
    ...
    void )
{
    ...
    for ( ; i < XI_ARRAYSIZE( array_of_test_case ); ++i )
    {
        ...
        xi_data_desc_t* buffer =
            xi_make_empty_desc_alloc( message_len - payload_size );
        XI_CHECK_MEMORY( buffer, local_state );

        xi_mqtt_serialiser_rc_t rc = xi_mqtt_serialiser_write(
