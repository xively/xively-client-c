...
void xi_utest_local__add_handler__evtd(
	  xi_evtd_instance_t* evtd
	...
	, xi_event_handle_arg4_t arg4)
{
	...
	xi_event_handle_t evtd_handle = {
		...
		.handlers.h4 = { fn, arg1, arg2, arg3, arg4 },
		0
	};

	xi_evtd_execute_in(evtd, evtd_handle, 1);
}

xi_state_t xi_utest_local_action__timed(
	  xi_event_handle_arg1_t target_evtd
	, xi_event_handle_arg2_t exec_counter
	, xi_event_handle_arg3_t state
	, xi_event_handle_arg4_t seconds_since_epoch )
{
	if (target_evtd == NULL || exec_counter == NULL) return XI_STATE_OK;
	{
		uint32_t* counter = (uint32_t*)exec_counter;

		printf("%s, counter = %u, time = %ld\n", __func__, (unsigned int)*counter, time( 0 ) );
...
xi_state_t xi_utest_local_action__io_timed( xi_event_handle_arg1_t arg1 )
{
    ...
    xi_heap_element_t* event = timeout_element->timeout;
...
SKIP_XI_TT_TESTCASE( utest__xi_evtd_execute_in__time_function_misbehaviour_upon_core_switch__test_for_multicore_archs__time_is_measured_same_on_different_cores,
{
    ...
    xi_utest_local__add_handler__evtd( evtd
    	, &xi_utest_local_action__timed
    	, evtd
    	, &exec_counter
    	, state
...
XI_TT_TESTCASE( utest__xi_evtd_io_execute_in__delayed_execution__events_should_be_executed,
{
    ...
    for ( index = 0 ; index < 50 ; index++ )
    {
        ...
        xi_event_handle_t evtd_handle = {
                            ...
                            .handlers.h1 = { (xi_event_handle_func_argc1_ptr ) &xi_utest_local_action__io_timed , timeout_element },
                            0
        };

        // schedule handle execution
        event = xi_io_timeouts_create(
              evtd
            , evtd_handle
            , 10
            , vector );

        // store event in the timeout element, so handle can remove it from the collector
        timeout_element->timeout = event;
    ...
    for ( index = 0; index < vector->elem_no; ++index )
    {
        ...
        xi_heap_element_t* event = element.selector_t.ptr_value;
        xi_heap_element_t* test_event = timeout->timeout;
...
XI_TT_TESTCASE( utest__xi_evtd_io_restart__delayed_execution__events_should_be_restarted,
{
    ...
    for ( index = 0 ; index < 50 ; index++ )
    {
        ...
        if ( index < 25 )
        {
            xi_vector_push(
                  test_vector
                , XI_VEC_CONST_VALUE_PARAM( XI_VEC_VALUE_PTR( event ) ) );
        }
    }
...
XI_TT_TESTCASE( utest__xi_evtd_io_remove__delayed_execution__events_should_be_removed,
{
    ...
    for ( index = 0 ; index < 25; index++ )
    {
        ...
        xi_vector_elem_t element = test_vector->array[ index ];
        event = element.selector_t.ptr_value;

        xi_io_timeouts_remove(
              event
            , vector );
    }
