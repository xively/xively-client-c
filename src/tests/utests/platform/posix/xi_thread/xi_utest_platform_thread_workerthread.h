...
XI_TT_TESTCASE( utest__posix__xi_workerthread_create_instance__no_input__fields_are_initialized,
{
	...
	for (; counter_testcase < XI_ARRAYSIZE(test_cases_evtd_ptrs); ++counter_testcase)
	{
...
XI_TT_TESTCASE( utest__posix__xi_workerthread__add_onetype_handler_multiple_times__all_handlers_are_executed,
{
	...
	for (; id_handler_adds < XI_ARRAYSIZE(nb_handler_additions) - 4; ++id_handler_adds)
	{
		xi_workerthread_t* workerthread = xi_workerthread_create_instance( NULL );

		//const uint8_t sync_point_reached = xi_workerthread_wait_sync_point(workerthread);
		//tt_want_int_op(1, ==, sync_point_reached);

		uint32_t value_shared_between_threads = 111;

		xi_utest_local__add_handler__workerthread(workerthread->thread_evtd
...
#endif
		tt_want_int_op(111 + nb_handler_additions[id_handler_adds], ==, value_shared_between_threads);
	}

	xi_destroy_critical_section( &xi_uteset_local_action_store_cs );
})

XI_TT_TESTCASE( utest__posix__xi_workerthread__add_twotypes_handler_multiple_times__all_handlers_are_executed,
{
	const size_t nb_handler_additions_A[] = { 67, 4,  3, 7, 1, 0 };
	const size_t nb_handler_additions_B[] = {  7, 4, 22, 0, 6, 7 }; // keep arrays same sized!

	xi_init_critical_section( &xi_uteset_local_action_store_cs );

	size_t id_handler_adds = 0;
	for (; id_handler_adds < XI_ARRAYSIZE(nb_handler_additions_A) - 5; ++id_handler_adds)
	{
		xi_workerthread_t* workerthread = xi_workerthread_create_instance( NULL );
...
XI_TT_TESTCASE( utest__posix__xi_workerthread__usageof_secondary_evtd__one_or_two_handlers_are_executed,
{
	...
	size_t id_handler_adds = 0;
	for (; id_handler_adds < (xi_test_load_level ? XI_ARRAYSIZE(nb_handler_additions) : 3)
		 ; ++id_handler_adds)
	{
		xi_evtd_instance_t* evtd = xi_evtd_create_instance();
		...
		xi_utest_local__add_handler__workerthread(workerthread->thread_evtd_secondary
			, &xi_utest_local_action_decrease_by_11
			, (xi_event_handle_arg1_t)&value_shared_between_threads
			, nb_handler_additions[id_handler_adds]);

		while (	!XI_LIST_EMPTY( xi_event_handle_t, workerthread->thread_evtd_secondary->call_queue ) )
		{
		...
		printf("current number of handler additions: %lu, result = %u\n"
			...
			, value_shared_between_threads);
#endif
		tt_want_int_op(1111 - nb_handler_additions[id_handler_adds]*11, ==, value_shared_between_threads);
	}

	xi_destroy_critical_section( &xi_uteset_local_action_store_cs );
})

XI_TT_TESTCASE( utest__posix__xi_workerthread__usageof_both_evtds__handlers_are_executed_properly,
{
	...
	size_t id_handler_adds = 0;
	for (; id_handler_adds < (xi_test_load_level ? XI_ARRAYSIZE(nb_handler_additions) : 2)
		 ; ++id_handler_adds)
	{
		xi_evtd_instance_t* evtd = xi_evtd_create_instance();
		xi_workerthread_t* workerthread = xi_workerthread_create_instance( evtd );
		...
		printf("current number of handler additions: %lu, result = %u\n"
			...
			, value_shared_between_threads);
#endif
		tt_want_int_op(1111 + (1 - 11) * nb_handler_additions[id_handler_adds], == ,value_shared_between_threads);
	}
