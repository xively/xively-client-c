/* Copyright (c) 2003-2018, LogMeIn, Inc. All rights reserved.
 *
 * This is part of the Xively C Client library,
 * it is licensed under the BSD 3-Clause license.
 */

#include "tinytest.h"
#include "tinytest_macros.h"
#include "xi_tt_testcase_management.h"
#include "xi_macros.h"

#include "xi_memory_limiter.h"
#include "xi_memory_checks.h"
#include "xively.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
#include <time.h>

#ifndef XI_TT_TESTCASE_ENUMERATION__SECONDPREPROCESSORRUN

const size_t utest_memory_limiter_generic_test_samples[] = {3072, 0xFAFAFAFA, 0xFFFFFFFF};

const size_t utest_memory_limiter_expected_generic_results[] = {
    3072 - XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT,
    0xFAFAFAFA - XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT,
    0xFFFFFFFF - XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT};

void test()
{
    const size_t application_capacity_pre_alloc = xi_memory_limiter_get_current_limit(
        XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION );

    const size_t original_alloc_size   = 0xAA;
    const size_t realloc_size          = 0xAF;
    const size_t alloc_size_difference = realloc_size - original_alloc_size;

    void* ptr = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                         original_alloc_size, __FILE__, __LINE__ );
    tt_assert( NULL != ptr );

    const size_t application_capacity_after_alloc = xi_memory_limiter_get_current_limit(
        XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION );

    memset( ptr, 0xCA, original_alloc_size );
    unsigned char* matrix[0xAA];
    memset( matrix, 0xCA, sizeof( matrix ) );

    void* realloc_ptr = xi_memory_limiter_realloc(
        XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION, ptr, 0xAF, __FILE__, __LINE__ );
    tt_assert( NULL != realloc_ptr );

    const size_t capacity_difference =
        application_capacity_after_alloc -
        xi_memory_limiter_get_current_limit(
            XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION );

    tt_assert( alloc_size_difference == capacity_difference );
    tt_assert( memcmp( realloc_ptr, matrix, original_alloc_size ) == 0 );

    xi_memory_limiter_free( realloc_ptr );

    tt_assert( application_capacity_pre_alloc ==
               xi_memory_limiter_get_current_limit(
                   XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
    return;
end:;
}

#endif /* XI_TT_TESTCASE_ENUMERATION__SECONDPREPROCESSORRUN */

XI_TT_TESTGROUP_BEGIN( utest_memory_limiter )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_set_limit__valid_data_application__new_limit_set, {
        size_t i = 0;

        for ( ; i < XI_ARRAYSIZE( utest_memory_limiter_generic_test_samples ); ++i )
        {
            tt_want_int_op( XI_STATE_OK, ==,
                            xi_memory_limiter_set_limit(
                                utest_memory_limiter_generic_test_samples[i] ) );
            tt_want_int_op( xi_memory_limiter_get_capacity(
                                XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ),
                            ==, utest_memory_limiter_expected_generic_results[i] );
            tt_want_int_op( xi_memory_limiter_get_current_limit(
                                XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ),
                            ==, utest_memory_limiter_expected_generic_results[i] );
        }
    } )


XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__valid_data__current_limit_shrinked_down_of_an_allocation_size_return_state_ok,
    {
        /* size values */
        const size_t allocation_size = 128;
        const size_t allocation_footprint =
            allocation_size + sizeof( xi_memory_limiter_entry_t );
        const size_t memory_limit_size = XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT + 1024;

        /* assumptions */
        tt_assert( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT > allocation_footprint );

        xi_state_t memory_set_result = xi_set_maximum_heap_usage( memory_limit_size );
        tt_assert( memory_set_result == XI_STATE_OK );

        void* ptr = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                             allocation_size, __FILE__, __LINE__ );
        tt_assert( NULL != ptr );
        tt_assert( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) ==
                   memory_limit_size - allocation_size -
                       sizeof( xi_memory_limiter_entry_t ) );

        tt_assert( memory_limit_size - XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        tt_assert( memory_limit_size == xi_memory_limiter_get_capacity(
                                            XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) );


        void* ptr2 = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                              allocation_size, __FILE__, __LINE__ );
        tt_assert( NULL != ptr2 );
        tt_assert( memory_limit_size - 2 * allocation_size -
                       2 * sizeof( xi_memory_limiter_entry_t ) ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) );

        tt_assert( memory_limit_size - XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        tt_assert( memory_limit_size == xi_memory_limiter_get_capacity(
                                            XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) );

        xi_memory_limiter_free( ptr );
        xi_memory_limiter_free( ptr2 );

        const size_t new_memory_limit =
            XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT + allocation_footprint;
        xi_memory_limiter_set_limit( new_memory_limit );

        void* ptr3 =
            xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                     allocation_size, __FILE__, __LINE__ );
        tt_assert( NULL != ptr3 );

        tt_assert( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) == 0x0 );
        tt_assert( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) ==
                   XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT );
        tt_assert( xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) ==
                   allocation_size + sizeof( xi_memory_limiter_entry_t ) );
        tt_assert( new_memory_limit == xi_memory_limiter_get_capacity(
                                           XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) );

        xi_memory_limiter_free( ptr3 );

        /* put things back to normal */
        xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                     XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
        return;
    end:
        xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                     XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
    } )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__vald_data_allocate_more_than_capacity__return_NULL,
    {
        /* size values */
        const size_t allocation_size = XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT * 2;
        const size_t memory_limit_size =
            XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT + sizeof( xi_memory_limiter_entry_t );

        xi_state_t memory_set_result = xi_set_maximum_heap_usage( memory_limit_size );
        tt_assert( memory_set_result == XI_STATE_OK );

        tt_assert( NULL ==
                   xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                            allocation_size, __FILE__, __LINE__ ) );
        memory_set_result =
            xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                         XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
        tt_assert( memory_set_result == XI_STATE_OK );
        return;
    end:
        xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                     XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
        ;
    } )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory_system__vald_data_allocate_more_than_capacity__return_NULL,
    {
        /* size values */
        const size_t allocation_size = XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT * 2;
        const size_t memory_limit_size =
            XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT + sizeof( xi_memory_limiter_entry_t );

        xi_state_t memory_set_result = xi_set_maximum_heap_usage( memory_limit_size );
        tt_assert( memory_set_result == XI_STATE_OK );

        tt_assert( NULL ==
                   xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                            allocation_size, __FILE__, __LINE__ ) );
        memory_set_result =
            xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                         XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
        tt_assert( memory_set_result == XI_STATE_OK );
        return;
    end:
        xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                                     XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT );
        ;
    } )


XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__wrong_data__return_invalid_parameter, {
        void* ptr = xi_memory_limiter_alloc(
            XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION, 0, __FILE__, __LINE__ );
        tt_assert( NULL != ptr );

        xi_memory_limiter_free( ptr );
    end:;
    } )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__valid_data_realloc_more__realloc_previously_allocated_memory,
    { test(); } )


XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__valid_data_realloc_less__realloc_previously_allocated_memory,
    {
        const size_t application_capacity_pre_alloc = xi_memory_limiter_get_current_limit(
            XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION );

        const size_t original_alloc_size = 0xAA;
        const size_t original_alloc_size_footprint =
            original_alloc_size + sizeof( xi_memory_limiter_entry_t );
        const size_t realloc_size           = 0xAF;
        const size_t realloc_size_footprint = 0xAF + sizeof( xi_memory_limiter_entry_t );

        const size_t expected_new_application_limit_alloc =
            application_capacity_pre_alloc - original_alloc_size_footprint;
        const size_t expected_new_application_limit_realloc =
            application_capacity_pre_alloc - realloc_size_footprint;

        void* ptr =
            xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                     original_alloc_size, __FILE__, __LINE__ );

        tt_assert( NULL != ptr );
        tt_assert( expected_new_application_limit_alloc ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        memset( ptr, 0xCA, original_alloc_size );
        unsigned char* matrix[0xAA];
        memset( matrix, 0xCA, sizeof( matrix ) );

        void* realloc_ptr =
            xi_memory_limiter_realloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION, ptr,
                                       realloc_size, __FILE__, __LINE__ );

        tt_assert( NULL != realloc_ptr );
        tt_assert( expected_new_application_limit_realloc ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
        tt_assert( memcmp( realloc_ptr, matrix, original_alloc_size ) == 0 );

        xi_memory_limiter_free( realloc_ptr );

        tt_assert( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) ==
                   application_capacity_pre_alloc );

        return;
    end:;
    } )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_free_memory__valid_data_free_previously_allocated_value__return_state_ok,
    {
        const size_t application_capacity = xi_memory_limiter_get_current_limit(
            XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION );
        const size_t alloc_size = 0xAA;
        const size_t alloc_size_footprint =
            alloc_size + sizeof( xi_memory_limiter_entry_t );
        const size_t application_limit_post_alloc =
            application_capacity - alloc_size_footprint;

        void* ptr =
            xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                     alloc_size, __FILE__, __LINE__ );
        tt_assert( NULL != ptr );
        tt_assert( application_limit_post_alloc ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        xi_memory_limiter_free( ptr );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        ptr = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                       application_capacity -
                                           sizeof( xi_memory_limiter_entry_t ),
                                       __FILE__, __LINE__ );
        tt_assert( NULL != ptr );
        tt_assert( 0x0 == xi_memory_limiter_get_current_limit(
                              XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        xi_memory_limiter_free( ptr );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
        tt_assert( application_capacity ==
                   xi_memory_limiter_get_capacity(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );

        return;
    end:;
    } )

XI_TT_TESTCASE( utest__xi_memory_limiter_reset_the_limiter, {
    xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT +
                                 XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT + 1 );
    tt_want_int_op( xi_is_whole_memory_deallocated(), >, 0 );
    xi_memory_limiter_set_limit( XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT +
                                 XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT );
} )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__vald_data_proper_capacities__return_proper_capacity,
    {
        tt_int_op( XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT +
                       XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT,
                   ==, xi_memory_limiter_get_current_limit(
                           XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ) );
        tt_int_op( XI_MEMORY_LIMITER_APPLICATION_MEMORY_LIMIT, ==,
                   xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) );
    end:;
    } )

XI_TT_TESTCASE(
    utest__xi_memory_limiter_allocate_memory__vald_data_allocate_system_and_application_to_capacity__return_NOT_NULL,
    {
        const size_t allocation_overhead = sizeof( xi_memory_limiter_entry_t );
        const size_t system_to_alloc =
            XI_MEMORY_LIMITER_SYSTEM_MEMORY_LIMIT - allocation_overhead;

        const size_t application_to_alloc =
            xi_memory_limiter_get_capacity(
                XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ) -
            allocation_overhead;

        void *ptr1 = NULL, *ptr2 = NULL;
        ptr1 = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION,
                                        application_to_alloc, __FILE__, __LINE__ );

        tt_int_op( xi_memory_limiter_get_allocated_space(), ==,
                   application_to_alloc + allocation_overhead );

        tt_ptr_op( NULL, !=, ptr1 );
        ptr2 = xi_memory_limiter_alloc( XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM,
                                        system_to_alloc, __FILE__, __LINE__ );
        tt_ptr_op( NULL, !=, ptr2 );
        tt_int_op( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_SYSTEM ),
                   ==, 0 );

        tt_int_op( xi_memory_limiter_get_current_limit(
                       XI_MEMORY_LIMITER_ALLOCATION_TYPE_APPLICATION ),
                   ==, 0 );

    end:
        xi_memory_limiter_free( ptr1 );
        xi_memory_limiter_free( ptr2 );
    } )

XI_TT_TESTGROUP_END

#ifndef XI_TT_TESTCASE_ENUMERATION__SECONDPREPROCESSORRUN
#define XI_TT_TESTCASE_ENUMERATION__SECONDPREPROCESSORRUN
#include __FILE__
#undef XI_TT_TESTCASE_ENUMERATION__SECONDPREPROCESSORRUN
#endif
