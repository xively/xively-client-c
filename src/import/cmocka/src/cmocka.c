...
#include <sys/time.h>  // Not included/needed in IAR EWARM baremetal environment...
#endif
...
static int get_symbol_value(
    ...
    if (list_find(head, symbol_name, symbol_names_match, &target_node)) {
        ...
        cm_print_error( "Warning: No entries for symbol %s.\n", symbol_name );
    }
    return 0;
}
...
static int value_in_set_display_error(
    ...
    assert_non_null(check_integer_set);
    {
        ...
        cm_print_error("%" "lu" " is %sin the set (", value,
                       invert ? "" : "not ");
        for (i = 0; i < size_of_set; i++) {
            cm_print_error("%" "lu" ", ", set[i]);
        }
        cm_print_error(")\n");
    }
    return 0;
...
#define XI_CM_PRINT_ERROR_COLORED( ansi_color, text ) \
    print_error( ansi_color "%s" ANSI_COLOR_RESET, text );

#define XI_CM_PRINT_MESSAGE_COLORED( ansi_color, text ) \
    print_message( ansi_color "%s" ANSI_COLOR_RESET, text );
...
static void cmprintf_group_finish_standard(size_t total_executed,
                                           ...
                                           size_t total_errors,
                                           struct CMUnitTestState *cm_tests,
                                           size_t milliseconds_elapsed)
{
    size_t i;

    XI_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[==========]");
    print_message(" %u test(s) run. (%lu ms)\n", (unsigned)total_executed, milliseconds_elapsed);
    XI_CM_PRINT_ERROR_COLORED( ANSI_COLOR_GREEN, "[  PASSED  ]");
    print_error(" %u test(s).\n",
                (unsigned)(total_passed));

    if (total_failed) {
        XI_CM_PRINT_ERROR_COLORED( ANSI_COLOR_RED, "[  FAILED  ]");
        print_error(" %"PRIdS " test(s), listed below:\n", total_failed);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
...
static void cmprintf_standard_elapsedtime(enum cm_printf_type type,
                                          ...
                                          size_t milliseconds_elapsed)
{
    ...
    case PRINTF_TEST_START:
        XI_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[ RUN      ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_SUCCESS:
        XI_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_GREEN, "[       OK ]");
        print_message(" %s (%lu ms)\n", test_name, milliseconds_elapsed);
        break;
    case PRINTF_TEST_FAILURE:
        if (error_message != NULL) {
            print_error("%s\n", error_message);
        }
        XI_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_RED, "[  FAILED  ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_SKIPPED:
        XI_CM_PRINT_MESSAGE_COLORED( ANSI_COLOR_YELLOW, "[  SKIPPED ]");
        print_message(" %s\n", test_name);
        break;
    case PRINTF_TEST_ERROR:
        ...
        XI_CM_PRINT_ERROR_COLORED( ANSI_COLOR_RED, "[  ERROR   ]");
        print_error(" %s\n", test_name);
        break;
    }
}
...
static void cmprintf_elapsedtime(enum cm_printf_type type,
                                 size_t test_number,
                                 const char *test_name,
                                 const char *error_message,
                                 size_t milliseconds_elapsed)
{
    ...
    switch (output) {
    case CM_OUTPUT_STDOUT:
        cmprintf_standard_elapsedtime(type, test_name, error_message, milliseconds_elapsed);
        break;
    case CM_OUTPUT_SUBUNIT:
...
static void cmprintf(enum cm_printf_type type,
                     ...
                     const char *error_message)
{
    cmprintf_elapsedtime(type, test_number, test_name, error_message, 0);
}
                            ...
                            CMFixtureFunction group_teardown)
{
    ...
    if (rc == 0) {
        ...
        for (i = 0; i < num_tests; i++) {
            ...
            if (rc == 0) {
                switch (cmtest->status) {
                    case CM_TEST_PASSED:
                        cmprintf_elapsedtime(PRINTF_TEST_SUCCESS,
                                 test_number,
                                 cmtest->test->name,
                                 cmtest->error_message,
                                 deltatime_testcase);
                    ...
                    case CM_TEST_SKIPPED:
                        ...
                        break;
                    case CM_TEST_FAILED:
                        cmprintf_elapsedtime(PRINTF_TEST_FAILURE,
                                 test_number,
                                 cmtest->test->name,
                                 cmtest->error_message,
                                 deltatime_testcase);
    ...
    if (group_teardown != NULL) {
        cmocka_run_group_fixture("cmocka_group_teardown",
                                 NULL,
                                 group_teardown,
                                 &group_state,
                                 group_check_point);
...
int _run_group_tests(const UnitTest * const tests, const size_t number_of_tests)
{
    ...
    const char** failed_names = (const char**)malloc(number_of_tests *
                                       sizeof(*failed_names));
    void **current_state = NULL;
    TestState group_state = {NULL, NULL};
