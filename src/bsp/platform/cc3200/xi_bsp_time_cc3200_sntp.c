...
static sntp_status_t sntp_status_entries[] =
  {
    SNTP_STATUS_TABLE
  };
    ...
    do
      {
        ...
        if ((status = setsockopt (s, SOL_SOCKET, SO_RCVTIMEO, &rcv_to, sizeof(rcv_to))) != 0)
          {
            errno = status;
           ...
           *  Send SNTP request
           */
        if ((status = sendto (s, sntp_request, sizeof(sntp_request), 0, (struct sockaddr *)&ntp_sa, sa_len)) < 0)
          {
            errno = status;
           ...
           */
        status = recvfrom (s, sntp_response, sizeof(sntp_response), 0, (struct sockaddr *)&ntp_sa, &sa_len);
        ...
        if (t > 0)
          {
            ...
            printf ("SNTP_G: %d, %s->0x%08X\n",
                    sntp_time_posix(),
                    ntp_server_names[ntp_server_index],
                    server_addr);
